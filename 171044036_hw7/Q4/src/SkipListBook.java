 ...
 */
public class SkipListBook<E extends Comparable<E>>  implements SearchTree<E> {
...
/*</listing>*/
    ...
    public E delete(E target) {
        ...
        for (int i = current.links.length-1; i>=0; i--){
            while(current.links[i] !=null && (current.links[i].data.compareTo(target) <= 0)){
                if ((current.links[i].data.compareTo(target) == 0)){
                    deleted= current.data;
                    current.links[i] = current.links[i].links[i];
                    size--;
                }
                current = current.links[i];
            }
    ...
    public boolean add(E item) {
        int level= logRandom();
        SLNode<E> newDatum = new SLNode<E>(level,item);
        SLNode<E> current = head;
        for (int i = current.links.length-1; i>=0; i--){
            ...
            if(i<=level-1){
                newDatum.links[i] = current.links[i];
                current.links[i] = newDatum;
            }
        }
        size++;
